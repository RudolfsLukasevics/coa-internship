...
#doesn't work --- tried to turn cells with text as.character and cells with dates as.Date
#CombinedCareerConnections <- CombinedCareerConnections %>%
 # mutate(Application_Due = case_when(grepl("\\s", Application_Due) | grepl("/", Application_Due) ~ as.character(Application_Due), TRUE ~ as.Date(Application_Due, format = "%b %d", tryFormats = c("%b %d", "%b %Y"))))
